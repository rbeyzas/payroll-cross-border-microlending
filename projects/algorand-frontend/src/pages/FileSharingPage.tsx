...
const FileSharingPage: React.FC = () => {
  ...
  const addToHistory = (entry: {
    ...
    setFileHistory((prev) => [historyEntry, ...prev])
  }
        ...
        if (status === 'connected') {
          ...
          webrtcTransfer
            ...
            .then(() => {
              ...
              // Auto-hide progress after 10 seconds, then show success for 5 seconds
              setTimeout(() => {
        ...
        {transferSuccess && (
          <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-6">
            <div className="flex items-center">
              ...
              <p className="text-green-800 font-medium">File sent successfully!</p>
            </div>
          </div>
        )}
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8">
          ...
          {currentView === 'receive' && (
            <div className="space-y-6">
              ...
              {pendingRequests.length === 0 && receivedFiles.length === 0 ? (
                ...
                <div className="space-y-4">
                  ...
                  {receivedFiles.length > 0 && (
                    <div className="mb-6">
                      <h3 className="text-lg font-semibold text-gray-900 mb-4">Received Files</h3>
                            ...
                            <div className="flex items-center justify-between">
                              <div className="flex items-center">
                                ...
                                <div>
                                  ...
                                  <p className="text-green-600 text-sm">{FileUtils.formatFileSize(file.size)}</p>
                                </div>
                              </div>
                              <button
                                onClick={() => {
                  ...
                  {pendingRequests.length > 0 && (
                    <div>
                      <h3 className="text-lg font-semibold text-gray-900 mb-4">Pending File Requests</h3>
                      <div className="space-y-4">
          ...
          {currentView === 'history' && (
            <div className="space-y-6">
              <div className="flex justify-between items-center">
                <h2 className="text-2xl font-bold text-gray-900">File Transfer History</h2>
                <button
                  onClick={() => {
                    // Clear history
                    setFileHistory([])
                    ...
                    <div key={entry.id} className="border border-gray-200 rounded-lg p-6">
                      <div className="flex justify-between items-start mb-4">
                        <div className="flex items-center">
                          <div
                            className={`w-10 h-10 rounded-full flex items-center justify-center mr-4 ${
                              entry.type === 'sent' ? 'bg-blue-100 text-blue-600' : 'bg-green-100 text-green-600'
                            }`}
                          >
                            {entry.type === 'sent' ? (
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-500">
                              {entry.type === 'sent' ? 'Sent to' : 'Received from'}: {entry.recipient || entry.sender || 'Unknown'}
                            </p>
                          </div>
                        </div>
                        <div className="text-right">
                          ...
                          <p className="text-sm text-gray-500 mt-1">{new Date(entry.timestamp).toLocaleString()}</p>
                        </div>
                      </div>

                      <div className="grid md:grid-cols-3 gap-4 text-sm">
                        <div>
                          <span className="font-medium text-gray-700">Size:</span> {FileUtils.formatFileSize(entry.fileSize)}
                        </div>
                        <div>
                          <span className="font-medium text-gray-700">Type:</span> {entry.fileType}
                        </div>
                        <div>
                          <span className="font-medium text-gray-700">Direction:</span>
                          <span className={`ml-1 ${entry.type === 'sent' ? 'text-blue-600' : 'text-green-600'}`}>

...
1. **Build Contracts**: `algokit project run build` compiles all smart contracts. You can also specify a specific contract by passing the name of the contract folder as an extra argument.
   For example: `algokit project run build -- payroll_app` will only build the `payroll_app` contract.
2. **Deploy**: Use `algokit project deploy localnet` to deploy contracts to the local network. You can also specify a specific contract by passing the name of the contract folder as an extra argument.
   For example: `algokit project deploy localnet -- payroll_app` will only deploy the `payroll_app` contract.
...
By default the template creates contracts in the `smart_contracts` directory. To add a new contract:
...
2. Decide what values you want to use for the `allowUpdate` and `allowDelete` parameters specified in your contract's `deploy-config.ts` file.
